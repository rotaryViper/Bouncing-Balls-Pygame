...
# Version 2
# Changes:
# Use groups instead of lists
# Moved everything into a main function
# There was no difference in performance :(
# Multiprocessing is too confusing to implement
# Created 20/6//2022
...
def main():
    ...
    ballGroup.add(Ball(ballImg, [0,0], [velLimit,velLimit]))
    ...
    while True:
        ...
        if spawnBall == True: ballGroup.add(Ball(ballImg, list(mousePos), randVelvector(velLimit)))

        # If player hits R, empty the ballList
        if key[pygame.K_r]: ballGroup.empty()

        ballGroup.update(ballGroup.sprites())
        ...
        BallCountTxt = comicSans.render(str(len(ballGroup.sprites())), False, white)
